...
| PC817 `K` (pin 2) | **680 Ω** → loose test wire (touch to `GND` = "gate open") | purple/black |

The 680 Ω LED resistor sits on the cathode side here; either side of the LED
works — it must exist **exactly once** in that loop (470 Ω–1 kΩ all fine,
...
| 12 | S.C.A. **right screw (+)** → **10 kΩ inline** → PC817 leg 1 `A` | 30 cm | bare / solder to leg |
...
The opto path is first tested live at the gate (S.C.A. gives it 33 V/10 kΩ).
...
1. Breaker ON → Robus boots normally (BlueBUS LED blinks 1×/s), ESP32 LED on.
2. Remote still opens the gate → nothing regressed.
3. Phone on WiFi → `http://kapija.local` → login → button moves the gate.
4. On the Robus board set Level-2 function **L4 = "on if leaf is open"**
   (manual §7.2.3, Table 14) → app state matches reality; if inverted or
